...
type Props = {
  color: Color;
  onPress: (from: Square, to: Square, promo: Omit<PieceSymbol, 'p' | 'k'>)=>void,
  move: Move,
  cancel: ()=>void
}
...
const Promote = ({color, onPress, move,cancel}: Props)=>(
  <View style = {styles.container}>
    <Pressable style = {styles.box} onPress = {()=>onPress(move.from, move.to, 'q')}>
      <Image style = {styles.image} source = {chooseImage('q', color)}></Image>
    </Pressable>
    <Pressable style = {styles.box} onPress = {()=>onPress(move.from, move.to, 'r')}>
      <Image style = {styles.image} source = {chooseImage('r', color)}></Image>
    </Pressable>
    <Pressable style = {styles.box} onPress = {()=>onPress(move.from, move.to, 'b')}>
      <Image style = {styles.image} source = {chooseImage('b', color)}></Image>
    </Pressable>
    <Pressable style = {styles.box} onPress = {()=>onPress(move.from, move.to, 'n')}>
      <Image style = {styles.image} source = {chooseImage('n', color)}></Image>
    </Pressable>
    <Pressable style = {styles.box} onPress = {cancel}>
